...
class OrderExecutor:
    ...
    def execute(self, trade: dict) -> bool:
        ...
        # Fetch LTP from Fyers if entry_price not provided
        if entry_price <= 0:
            try:
                ...
                logger.info(f"Fetched LTP for {symbol}: Rs{entry_price}")
    ...
    def monitor_positions(self, fyers_connector=None):
        ...
        # Check SL and Target for all positions
        ...
        for symbol, pos in list(self.open_positions.items()):
            try:
                ...
                # Check Target hit
                    ...
                    logger.success(f"TARGET HIT: {symbol} @ Rs{ltp:.2f}")
                    self._exit_position(symbol, pos, ltp, "TARGET_HIT")
    ...
    def _save_trade(self, symbol, strike, option_type, expiry,
        ...
        try:
            ...
            t = Trade(
                ...
                strategy_used   = f"SUPERTREND_{timeframe}MIN",
                reason          = reasoning,
